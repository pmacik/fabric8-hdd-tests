...
if [ -f $LOCUST_FILE ]; then
	...
	CMD="nohup locust -f $JOB_BASE_NAME-$BUILD_NUMBER-locustfile.py --no-web --print-stats --no-reset-stats --clients=$USERS --hatch-rate=$USER_HATCH_RATE --csv=$JOB_BASE_NAME-$BUILD_NUMBER-report 2>$JOB_BASE_NAME-$BUILD_NUMBER-locust-master.log > $JOB_BASE_NAME-$BUILD_NUMBER-locust-master.log &";
